...
public class ProductoExporterPDF {
	...
	private void escribirCabeceraDeLaTabla(PdfPTable tabla) {
		...
		celda.setPhrase(new Phrase("Decripcion", fuente));
		tabla.addCell(celda);

		celda.setPhrase(new Phrase("TotalExistencia", fuente));
		tabla.addCell(celda);
		
		celda.setPhrase(new Phrase("PrecioMt2", fuente));
		tabla.addCell(celda);
	...
	private void escribirDatosDeLaTabla(PdfPTable tabla) {
		for (Producto producto : listaProductos) {
			...
			tabla.addCell(String.valueOf(producto.getPreciomt2()));
			tabla.addCell(String.valueOf(producto.getTotalexistencia()));
